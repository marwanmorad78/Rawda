from django.urls import path

from .views import (
    CategoryDeleteView,
    CategoryManageView,
    CategoryUpdateView,
    CenterStatusManageView,
    CustomerDetailView,
    CustomerListView,
    DashboardHomeView,
    DashboardOrderAcceptView,
    DashboardOrderAdvanceView,
    DashboardOrderDetailView,
    DashboardOrderRejectView,
    DashboardOrdersView,
    ManagerNewOrdersView,
    DeliveryAreaDeleteView,
    DeliveryAreaExcelTemplateView,
    DeliveryAreaExcelUploadView,
    DeliveryAreaManageView,
    DeliveryAreaUpdateView,
    DollarPriceManageView,
    ManagerLoginView,
    ManagerLogoutView,
    PendingOrdersView,
    ProductDeleteView,
    ProductExcelTemplateView,
    ProductExcelUploadView,
    ProductListView,
    ProductManageView,
    ProductUpdateView,
    PromotionDeleteView,
    PromotionManageView,
    PromotionUpdateView,
    SalesReportExcelExportView,
    SiteSettingsManageView,
)

app_name = "dashboard"

urlpatterns = [
    path("", DashboardHomeView.as_view(), name="index"),
    path("login/", ManagerLoginView.as_view(), name="login"),
    path("logout/", ManagerLogoutView.as_view(), name="logout"),
    path("customers/", CustomerListView.as_view(), name="customers"),
    path("customers/<int:pk>/", CustomerDetailView.as_view(), name="customer-detail"),
    path("pending-orders/", PendingOrdersView.as_view(), name="pending-orders"),
    path("pending-orders/<int:pk>/", DashboardOrderDetailView.as_view(), name="pending-order-detail"),
    path("pending-orders/<int:pk>/accept/", DashboardOrderAcceptView.as_view(), name="pending-order-accept"),
    path("pending-orders/<int:pk>/advance/", DashboardOrderAdvanceView.as_view(), name="pending-order-advance"),
    path("pending-orders/<int:pk>/reject/", DashboardOrderRejectView.as_view(), name="pending-order-reject"),
    path("orders/", DashboardOrdersView.as_view(), name="orders"),
    path("orders/check-new/", ManagerNewOrdersView.as_view(), name="orders-check-new"),
    path("orders/export-excel/", SalesReportExcelExportView.as_view(), name="orders-sales-export"),
    path("categories/", CategoryManageView.as_view(), name="categories"),
    path("categories/<int:pk>/edit/", CategoryUpdateView.as_view(), name="category-edit"),
    path("categories/<int:pk>/delete/", CategoryDeleteView.as_view(), name="category-delete"),
    path("products/", ProductManageView.as_view(), name="products"),
    path("products/list/", ProductListView.as_view(), name="product-list"),
    path("products/excel-template/", ProductExcelTemplateView.as_view(), name="product-excel-template"),
    path("products/upload-excel/", ProductExcelUploadView.as_view(), name="product-excel-upload"),
    path("products/<int:pk>/edit/", ProductUpdateView.as_view(), name="product-edit"),
    path("products/<int:pk>/delete/", ProductDeleteView.as_view(), name="product-delete"),
    path("promotions/", PromotionManageView.as_view(), name="promotions"),
    path("promotions/<int:pk>/edit/", PromotionUpdateView.as_view(), name="promotion-edit"),
    path("promotions/<int:pk>/delete/", PromotionDeleteView.as_view(), name="promotion-delete"),
    path("delivery-areas/", DeliveryAreaManageView.as_view(), name="delivery-areas"),
    path("delivery-areas/excel-template/", DeliveryAreaExcelTemplateView.as_view(), name="delivery-area-excel-template"),
    path("delivery-areas/upload-excel/", DeliveryAreaExcelUploadView.as_view(), name="delivery-area-excel-upload"),
    path("delivery-areas/<int:pk>/edit/", DeliveryAreaUpdateView.as_view(), name="delivery-area-edit"),
    path("delivery-areas/<int:pk>/delete/", DeliveryAreaDeleteView.as_view(), name="delivery-area-delete"),
    path("center-status/", CenterStatusManageView.as_view(), name="center-status"),
    path("dollar-price/", DollarPriceManageView.as_view(), name="dollar-price"),
    path("settings/", SiteSettingsManageView.as_view(), name="settings"),
]
